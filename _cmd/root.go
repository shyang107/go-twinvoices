...
func init() {
	...
	// RootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.cdemo.yaml)")
	...
	RootCmd.Flags().BoolVarP(&verbose, "verbos", "v", false, "Activate display of messages on console (defaukt is false)")
	RootCmd.Flags().StringVarP(&casefile, "case", "c",
		inv.DefaultConfig.CaseFilename,
		util.Sf("case-setting file (default is %v)", inv.DefaultConfig.CaseFilename))
