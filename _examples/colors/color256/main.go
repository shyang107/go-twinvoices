...
func printAll256Colors() {
	...
	// ptype(code)
	for i := 0; i < 16; i++ {
		bgcode := pencil.ColorCode(i)
		if i < 8 {
			ansi256.FBPrintfFunc(ansi256.White, bgcode)("    %2d   ", i)
		} else {
			if i == 8 {
				fmt.Print(" ")
			}
			ansi256.FBPrintfFunc(ansi256.Black, bgcode)("    %2d   ", i)
		}
	}
	fmt.Println()
	for i := 0; i < 6; i++ {
		for j := 0; j < 36; j++ {
			code := 16 + i*36 + j
			bgcode := pencil.ColorCode(code)
			if j < 18 {
				ansi256.FBPrintf(ansi256.White, bgcode, "%3d ", code)
			} else {
				if j == 18 {
					fmt.Print(" ")
				}
				ansi256.FBPrintf(ansi256.Black, bgcode, "%3d ", code)
			}
		}
		fmt.Println()
	}
	for j := 0; j < 24; j++ {
		...
		bgcode := pencil.ColorCode(code)
		if j < 12 {
			ansi256.FBPrintfFunc(ansi256.White, bgcode)(" %3d  ", code)
		} else {
			if j == 12 {
				fmt.Print(" ")
			}
			ansi256.FBPrintfFunc(ansi256.Black, bgcode)(" %3d  ", code)
		}
	}
	fmt.Println()
...
func test1() {
	var code pencil.Attribute = 131 << 8
	fgcode := pencil.Foreground | code
	bgcode := pencil.Background | code
	fmt.Println("Foreground = ", pencil.Foreground, "Background = ", pencil.Background, "; code = ", code)
	fmt.Println("Foreground | code = fgcode = ", fgcode)
	fmt.Printf("  %016b\n| %016b\n= %016b\n", pencil.Foreground, code, fgcode)
	fmt.Println("Foreground & fgcode = ", pencil.Foreground&fgcode)
	fmt.Printf("  %016b\n& %016b\n= %016b\n", pencil.Foreground, fgcode, pencil.Foreground&fgcode)
	fmt.Println("Background & fgcode = ", pencil.Background&fgcode)
	fmt.Println("Foreground ^ fgcode = ", pencil.Foreground^fgcode)
	fmt.Println("Background | code = bgcode = ", bgcode)
	fmt.Println("Background & bgcode = ", pencil.Background&bgcode)
	fmt.Println("Foreground & bgcode = ", pencil.Foreground&bgcode)
	fmt.Println("Background ^ bgcode = ", pencil.Background^bgcode)
